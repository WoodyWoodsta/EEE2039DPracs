...
// void initTIM6Auto(uint32_t userFreq, uint16_t userPSC, int8_t interruptEnable, int8_t bufferEnable); // Initialise Timer 6 [FREQUENCY, PSC, INTERUPT ENABLE (YES/NO), BUFFER ENABLE (YES/NO)]

void delayms(uint32_t length); // Millisecond delay function [Delay Length]
void delaypointms(uint32_t length); // Point 1 millisecond delay function [Delay Length]

uint16_t getPot(int8_t POT); // Get the pot value [POT#]
int8_t getPB(int8_t button); // Get the button state [SW#]
// uint32_t calcARR(uint16_t PSCVal, uint32_t freq); // Calculate the ARR for TIM6 [PSC, FREQUENCY]
