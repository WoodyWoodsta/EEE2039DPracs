...
// void initTIM6Auto(uint32_t userFreq, uint16_t userPSC, int8_t interruptEnable, int8_t bufferEnable) { // Initialise the TIM6 given a frequncy
//   RCC -> APB1ENR |= RCC_APB1ENR_TIM6EN; // Enable the RCC for TIM6
...
//   TIM6 -> ARR = calcARR(userPSC, userFreq); // Set the ARR to the value specified
...
// uint32_t calcARR(uint16_t PSCVal, uint32_t freq) { // Calculate the ARR for TIM6 given the PSC and a frequency
//   float period = 1/freq; // Calculate the period
//   uint32_t ARRVal = ((period*8000000)/(PSCVal + 1)); // Calculate the ARR
  
//   if ((ARRVal > 0xFF)||(ARRVal == 0)) {
//     programError();
//   }
